...
| `allNotificationsProvider` (`Provider<AsyncValue<List<AppNotification>>>`) — **SSOT**. 두 출처 병합(최신순). 그룹 축의 로딩/에러를 그대로 전파 | provider | `lib/shared/providers/group_notifications_provider.dart` | 3.3 |
...
| `nowProvider` (`Provider<DateTime>`) — **SSOT**. 날짜 파생 계산이 공유하는 단일 시각. 소비자가 각자 `DateTime.now()`를 읽으면 서로 다른 "오늘"로 판정한다 | provider | `lib/shared/providers/now_provider.dart` | 3.3 |
