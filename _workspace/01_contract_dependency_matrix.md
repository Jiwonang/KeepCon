...
작성일: 2026-07-06 · 최종 갱신: 2026-09-17 · 계약 버전: **v3.16** (non-breaking — `shareGifticon`이 원본의 현재 상태를 검사한다)
...
| `GifticonRepository.extendExpiry(String id, DateTime newExpiryDate)` → `Future<Gifticon>` — 유효기간 연장. 만료일을 **뒤로만** 옮기고(`isLaterExpiryDate` — 달력 일 단위), 저장된 `expired`는 `available`로 되돌린다(**전이 표의 유일한 예외**). `used`는 거부. ⚠️ **공유 중이면 부르지 말 것** — 스냅샷이 안 따라온다(`ShareRepository.extendSharedExpiry` 사용) | method | 〃 | 3.14 |
...
| `GroupNotificationType` (registered/expiringSoon/used/**expiryExtended**) | enum | `lib/shared/models/share.dart` | 3.14 |
...
| `ShareRepository.issueInviteCode({groupId})` → `Future<Group>` (방장 전용 · 발급 시점 + **5분**) | method | 〃 | 3.9 |
| `ShareRepository.watchSharedGifticons(groupId)` / `getSharedGifticons(groupId)` | method | 〃 | 2 |
| `ShareRepository.shareGifticon({groupId, Gifticon gifticon})` → `Future<SharedGifticon>` — **원본 상태 가드**: 넘긴 `gifticon`(화면의 스냅샷)이 아니라 `GifticonRepository`의 **현재** 원본을 다시 읽어 `available`이 아니면 `StateError`(아무것도 남기지 않음). 레코드의 표시 필드도 그 원본으로 만든다. 원본을 못 찾으면(데모 시드) 검사를 건너뛰고 스냅샷을 쓴다 | method | 〃 | 3.16 |
...
| `ShareRepository.extendSharedExpiry(id, DateTime newExpiryDate)` → `Future<SharedGifticon>` — **공유자만**(`sharedByUserId`) 연장할 수 있고, 스냅샷과 **원본을 함께** 옮긴다(순서: 원본 → 스냅샷). `used`는 거부, `inUse`·찜은 허용. 그룹에 `GroupNotificationType.expiryExtended` 알림을 남긴다 | method | 〃 | 3.14 |
...
| `ShareRepository.requestToJoin(credential)` → `Future<JoinRequest>` — `credential`은 **링크 토큰 또는 6자리 초대코드**. 모양(`isWellFormedInviteCode`)은 **첫 조회 대상의 힌트일 뿐**이고(v3.0 이전 6자리 링크 토큰이 살아 있다) 정본은 **어느 조회 문서에서 찾았는지**다. 만료는 **사용한 자격증명 기준**. ⚠️ **대기 중인 요청이 있으면 조용히 반환하지 않고 예외로 알린다**(아래 행) — 결정이 끝난 요청(거절·강퇴 후)은 되살아나 **정상 반환**이다 | method | 〃 | 3.13 |
| `InviteExpiredException(credential, {required bool isCode}) extends StateError` — 만료만 구별해 잡는 하위 타입. `isCode`는 구현체의 **폴백 조회 판정**이며(모양이 아니다) 만료 판정에 쓴 창과 같은 값이어야 한다 — 화면은 이 값으로 "새 코드를 요청"과 "링크 재발급을 요청"을 가른다 | class | `lib/shared/repositories/share_repository.dart` | 3.12 |
| `AlreadyGroupMemberException(groupId) extends StateError` — 행위자가 이미 멤버라 요청이 거부됐다. 화면이 "이미 초대된 회원입니다"로 구별해 안내한다(이 실패에는 **다음 행동이 없다** — 코드를 다시 받아 와도 같은 답이다). ⚠️ 만료 검사가 이 판정보다 **먼저** 온다 | class | 〃 | 3.13 |
| `JoinRequestAlreadyPendingException(request) extends StateError` — 대기 중인 요청이 이미 있어 **새로 만들지 않았다**(저장소 부작용은 없다 — 요청은 쌓이지 않는다). `request`는 **기존** 요청이다. 화면이 "이미 승인 요청을 보낸 그룹입니다"로 안내한다 — 조용히 반환하면 방금 보낸 것과 구별되지 않아 "보냈어요"를 다시 띄운다 | class | 〃 | 3.13 |
| `Group.inviteCode` / `inviteCodeExpiresAt`(짝 불변식) / `inviteCodeValidity`(5분) / `hasActiveInviteCode(now)` / `inviteCodeRemaining(now)` | field·const·method | `lib/shared/models/group.dart` | 3.9 |
| `newInviteCode({random})` → `String`(6자리, 선행 0 보존) / `isWellFormedInviteCode(v)` → `bool` | function | `lib/shared/util/invite_code.dart` | 3.9 |
...
11. **share 경계면 (v2 승격) — 공유 사용 동기화·행위자 식별**
   ...
   - **`shareGifticon`/`cancelShare`는 원본 상태 불변.** 공유/회수는 원본을 available로 유지한다.
     단 `shareGifticon`은 **원본이 지금 `available`일 때만** 공유한다(v3.16 — `getGifticonById`로
     다시 읽어 판정. 원본을 못 찾으면 건너뛴다).
   - **인스턴스 공유 필수.** `shareRepositoryProvider`는 `authRepositoryProvider`·
     ...
     main에 반영되지 않고, `shareGifticon`의 원본 상태 가드가 "원본 없음"으로 **조용히 건너뛴다**
     (#7과 동일 원인).
   ...
   - **표시 필드는 스냅샷.** `SharedGifticon`의 brand/productName/expiryDate/barcode는 공유 시점
     원본 스냅샷 — v3.16부터 호출자 값이 아니라 **다시 읽은 원본**(없으면 호출자 값). 다른 멤버는
     공유자 개인 목록 조회 불가. 원본 연결은 `gifticonId`. (만료일은 공유 뒤 `extendSharedExpiry`로만 뒤따른다)
...
13. **`myGroupsProvider` = SSOT (`lib/shared/providers/my_groups_provider.dart`, v2.5)**
   ...
   - **수명 = `autoDispose`.** scan은 일회성 플로우라 이탈 시 리스너가 남으면 안 되고,
     ...
     ✅ **배너 승격 완료(v3.11).** PR #137이 scan에 그룹 선택 타일을 넣으며 **두 번째 소비
     지점이 확정되어** share 페이지 폴더에서 `lib/shared/widgets/`로 옮기고 `Share` 접두를
     떼었다(공유 도메인 전용이 아니게 됐다). **이동·개명만이며 렌더링·동작은 불변**이다.
     ⚠️ **배선은 아직이다 — 현재 실소비자는 share 하나뿐이다**(`lib/features/scan`에 참조
     0건). scan·main·auth 연결은 각 담당의 후속 PR이며, 이 승격은 규칙 ③의 "실제 두 번째
     소비자"가 이미 생겨서가 아니라 소비 **지점**이 확정된 뒤 사본이 생기는 것을 막으려고
     앞당긴 것이다. 소비 지점은 share 6파일 9곳 + 테스트 1파일이고, 옛 이름은 `lib`·`test`에
     0건이다(`_workspace/qa_report.md`의 4곳은 날짜 박힌 과거 QA 스냅샷이라 **의도적으로
     남긴다** — 누락으로 읽고 개명하지 말 것).
...
| v3.8 | 2026-08-24 | **`joinGroup` 삭제 (⚠️ breaking).** 링크(토큰) 소지만으로 즉시 멤버가 되던 경로를 계약·양쪽 구현·테스트에서 걷어냈다. 호출부는 v3.1에서 이미 0이었지만, **계약에 남아 있는 한 같은 토큰으로 승인을 건너뛰는 우회로**다 — `@Deprecated`도 문서도 호출을 막지 못하므로 삭제가 유일한 게이트다. 옛 가드가 간 자리: 없는 토큰·만료·중복 요청 → `requestToJoin`, 정원 → `approveJoinRequest`(대기자가 자리를 선점하지 못하게 승인 시점으로 옮겼다). ⚠️ **'이미 멤버'는 의미와 순서가 둘 다 바뀌었다** — 옛 `joinGroup`은 no-op으로 현재 그룹을 반환했고 그 판정이 만료·정원보다 **우선**이었는데, 지금은 `StateError`이고 **만료 검사가 먼저**다(멤버가 24시간 지난 자기 링크를 누르면 '만료'로 거부된다). 그 동작 변경 자체는 v2.9에서 들어왔고 여기서는 대응 관계를 기록한다. **in-memory의 계약 위반 스텁도 함께 제거** — 없는 토큰에 가짜 그룹을 만들어 합류시켜, 오타 난 토큰이 성공한 참여처럼 보였다(리뷰에서 두 번 지적되고 "메서드를 지우는 단계에서 함께 정리한다"고 예고돼 있던 자리). 테스트의 `joinGroup` 사용 11곳은 성격을 갈라 처리했다 — 진짜 커버리지 4곳은 새 주인에게 옮기고, 셋업 7곳은 `joinHostGroup()` 헬퍼로 **실제 그룹 + 실제 승인**을 거치게 했다(옛 셋업은 존재하지 않는 토큰으로 멤버가 됐다 — 셋업이 거짓이면 그 위에 쌓은 단언도 거짓이다). **버그를 정상으로 단언하던 테스트 하나를 삭제**했다(`'미발견 코드는 데모용 가짜 그룹으로 합류'` → `'없는 토큰은 StateError'`). 소스 스캔 가드(`firebase_share_write_scope_test`)의 카나리아가 `joinGroup`이었어서 `approveJoinRequest`로 옮겼다 — 그대로 뒀으면 **가드가 조용히 무력해진다**. **[에이전트 리뷰 반영]** 🟠 이 행의 버전 번호를 조회하지 않고 `v3.2`로 새겼는데 그건 이미 알림 provider 승격에 배정된 번호였다(문서는 v3.7까지 와 있었다). 🟡 "이전"한 테스트 4건이 기존 `share_repository_join_request_test`와 중복임이 뮤테이션으로 드러나(가드 제거마다 2건 동시 실패) 제목이 거짓인 1건을 삭제하고 나머지는 각 파일 주제로 차별화했다. 🟡 `joinViaApproval` 헬퍼가 실패 시 세션을 방장으로 남겨 뒤따르는 단언을 조용히 방장 기준으로 평가시켰다(`try`/`finally`). 그 외 낡은 헤더·픽스처 정정. 규칙 검증 69/69, 테스트 540. | share, 그 외 페이지 영향 없음 |
| v3.9 | 2026-08-26 | **초대코드(6자리 OTP) 추가 — 링크와 나란한 두 번째 자격증명 (non-breaking).** 초대 경로를 둘로 나눴다: **링크**(카카오톡 등으로 보내는 것, 128비트 토큰·24시간)와 **초대코드**(옆사람에게 **불러주는** 것, 6자리 숫자·**5분**). 코드는 그룹의 속성이 아니라 방장이 발급 버튼을 누른 뒤 5분만 존재한다. (1) `Group.inviteCode`·`inviteCodeExpiresAt`는 **짝 불변식**(생성자 강제) — 한쪽만 채우면 "만료 없는 코드"나 "가리킬 코드가 없는 만료"가 생겨 `hasActiveInviteCode`가 판정할 수 없는 상태가 된다. 판정은 값의 유무가 아니라 **항상 시각과 함께** 한다(만료된 값은 그대로 남는다 — 그 시각에 문서를 고쳐 줄 주체가 없다). (2) `issueInviteCode`는 **방장 전용**이다. 정책이 아니라 **정리 권한** 때문 — 교체 시 옛 코드의 조회 문서를 지워야 하는데 그 삭제 권한이 규칙상 방장에게만 있어, 멤버에게 열면 옛 코드가 살아남아 '그룹당 활성 코드 하나' 불변식이 깨진다. 멤버의 초대 경로는 링크로 남는다. (3) **`inviteCodes/{code}` 컬렉션을 따로 뒀다** — `inviteTokens`에 섞지 못한 이유는 그 컬렉션의 쓰기 규칙이 `expiresAt == group.inviteExpiresAt`(24시간)를 **못박고** 있어서다. 한 컬렉션에 24시간짜리와 5분짜리를 섞으면 규칙이 어느 정본을 봐야 할지 알 수 없다. (4) ⚠️ **문서 키는 `shortInviteCode`다(`inviteCode`가 아니다)** — `inviteCode`는 링크 토큰의 **옛 이름**이라 레거시 문서가 그 키에 토큰을 담고 있고 매퍼가 폴백으로 읽는다. 같은 키를 쓰면 레거시 그룹의 **링크 토큰이 덮여 사라진다**(작업 중 발견해 분리). (5) **만료는 사용한 자격증명 기준이다 — 이 기능의 보안 전제 전체가 여기 걸려 있다.** 규칙의 `canRequestJoin`이 그룹의 `inviteExpiresAt` 하나로 판정하던 것을, 요청 문서가 싣고 온 `credential`의 만료를 보도록 바꿨다(`credentialGrants`). 안 그러면 5분짜리 코드로 얻은 `groupId`가 링크의 **남은 24시간 동안 계속 먹혀** "5분 만료"가 화면에만 있는 문구가 된다. `credential`은 경로 세그먼트로 쓰이므로 규칙에서 문자 집합·길이를 좁혔다. (6) 6자리가 안전한 근거는 "짧아서"가 아니라 **"잠깐이라서"**다 — `get`으로 10^6을 훑을 수 있지만 ①상시 활성 코드가 없고(그룹당 하나·5분) ②`joinRequests` 문서 id가 `{groupId}_{userId}`라 한 계정이 승인 큐를 도배할 수 없으며 ③규칙이 만료를 강제한다. 셋 중 하나라도 빠지면 방어가 무너진다. (7) 화면: 발급 버튼 + 1초 카운트다운(활성일 때만 틱 — 없으면 그릴 것이 바뀌지 않는데 매초 리빌드한다), 만료를 **구별한 안내**("만료된 초대코드예요") — 코드는 만료가 정상 경로라 뭉뚱그리면 사용자가 같은 코드를 계속 다시 입력한다. 판정은 문자열이 아니라 `InviteExpiredException` **타입**으로 한다. 기존 화면이 22자 링크 토큰을 '초대코드'라 부르던 자리를 진짜 코드가 가져갔다(같은 이름이 둘이면 어느 쪽을 부르는지 알 수 없다). 재발급 관련 문구는 전부 '링크'로 정정. 규칙 검증 **93/93**, 테스트 **589**. **[에이전트 리뷰 반영]** 🟠 **가장 아픈 지적은 레거시 6자리 링크 토큰이었다** — 링크 토큰이 128비트가 된 건 v3.0이고 **그 이전 토큰은 6자리 숫자였다.** 그 값들이 아직 살아 있는데(팀 공용 시드 `482913`, 데모 시드, 레거시 문서) 자격증명을 모양으로 **배타적으로** 골라서, 그 링크들이 통째로 '없는 코드'가 됐다. 보안 규칙의 `credentialGrants`는 두 컬렉션을 OR로 보므로 **클라이언트만 규칙보다 좁아 정상 링크를 스스로 거부**하고 있었고, 그래서 규칙 검증 93/93이 이 구멍을 못 잡았다. 코드를 먼저 보되 빗나가면 토큰도 보도록 고쳤다(우선순위는 코드 — 그래야 5분 창이 24시간 창에 흡수되지 않는다). 발급 시 레거시 토큰과의 충돌도 함께 피한다(내 그룹 것이어도 피한다 — 내 링크를 내 코드로 가리는 것도 그 링크를 죽인다). 하필 **내 테스트가 그 시드 토큰 값을 유효한 초대코드 예시로 쓰고 있어**, 두 공간이 겹친다는 증거가 테스트 안에 있었는데도 놓쳤다. 🟠 형제 누락 — `_cascadeDeleteGroupJoinArtifacts`는 정리 대상을 셋으로 늘렸는데 실패 시 복원(`_restoreInviteToken`)은 토큰만 되살렸다. 그룹 문서는 6자리를 계속 내걸고 조회 문서만 없는 상태가 남아, 방장 화면의 카운트다운이 아무도 풀 수 없는 값을 가리킨다 — `issueInviteCode`에서 내가 명시적으로 막아 둔 바로 그 어긋남을 형제 경로에 남긴 것이다. 🟡 옛 코드 삭제만 `try` 밖에 있어 그 단계가 실패하면 되돌리기 없이 활성 코드가 둘이 됐다. 🟡 `_groupToDoc` 주석이 `tx.update`의 병합 의미와 **반대로** 적혀 있었다("코드 없는 그룹을 되쓰면 필드가 사라진다" — `update`는 맵에 없는 필드를 지우지 않는다). 🟡 개명 잔존 9곳 — 특히 `_regenerate`의 doc가 "초대코드를 재발급한다"인데 같은 함수가 여는 다이얼로그는 "초대 링크 재발급 / 6자리 초대코드는 영향을 받지 않아요"였다(자기모순). `formatInviteExpiry`·딥링크 프리필·spec 문서까지 정리. 🟡 `shortInviteCode` 문서 매핑에 테스트가 **0건**이었다 — 리뷰어가 짝 판정을 깨는 뮤테이션으로 580건 전부 통과함을 증명했다. 매퍼 테스트 6건 추가(뮤테이션 재적용 시 2건 실패 확인). 🔵 SSOT 함수 가드 등록, 참여 시트 라벨·힌트 불일치. 테스트 **589**, 규칙 93/93. **[CodeRabbit 리뷰 반영]** actionable 4건. 🟡 **`InviteExpiredException`이 초대코드 전용이 아니라는 것**을 화면이 놓쳤다 — 계약이 `credential`을 "링크 토큰 또는 6자리 코드"로 정의하므로 만료된 **링크**도 같은 예외를 던지고, 이 시트는 딥링크로 열릴 때 링크 토큰을 미리 채운다. 그래서 링크가 만료된 사람에게 "새 **코드**를 요청하세요"라고 말했다 — 코드를 받아 와도 만료된 링크는 그대로라 **다음 행동이 틀린 안내**다. 입력값 모양으로 갈라 문구를 둘로 나눴다(에이전트 리뷰는 이걸 놓쳤다 — 다른 벤더의 시각이 값을 한 지점). 🟡 검증 케이스 `남의 그룹 코드를 돌려쓰기 → 차단`이 **의도한 조항을 거치지 않고** 403이었다: 대상 그룹에 `shortInviteCodeExpiresAt`가 없어 앞선 만료 일치 조항에서 필드 접근 오류로 죽은 것이고, 탈취 방지 조항은 실행조차 되지 않았다(통과하는데 이유가 틀린 케이스). 만료 정본이 맞는 두 번째 그룹을 준비해 고쳤고, **조항을 제거하는 뮤테이션으로 그 케이스만 200으로 뒤집히는 것을 확인**했다. 🟡 코드 문서 id가 고정값이라 에뮬레이터 재사용 시 앞선 실행의 문서가 남아 다음 실행이 '돌려쓰기 금지'에 막혔다 — 실행별 6자리로 바꾸고 정리 케이스도 더했다(규칙이 `^[0-9]{6}$`를 요구해 `${JR_RUN}` 접미를 붙일 수 없다). 🟡 테스트 이름이 검증하지 않는 시나리오를 주장했다 — `newGroup()`의 토큰은 22자라 6자리 코드와 같은 값이 될 수 없어 '폴백 우선순위'를 고정하지 못하는데 그렇게 이름 붙였다. 🔵 `issueInviteCode` 백엔드 실패 경로가 `share_action_failure_ui_test`의 축 밖에 있었다(형제 중 하나만 빠진 비대칭). 규칙 검증 **95/95**, 테스트 **591**. | share(생산·소비), 그 외 페이지 영향 없음 |
| v3.10 | 2026-08-29 | **v2.4의 scan 전환 기록 정정 (문서만 — 계약 무변경).** v2.4는 "scan 날짜 선택 버튼 라벨이 `YYYY-MM-DD` → `YYYY.MM.DD`로 바뀌었다"고 적었지만, 그 전환은 **렌더링되지 않던 `lib/features/scan/widgets/gifticon_form.dart`에만** 적용됐다. 실제 화면인 `scan_page.dart`의 인라인 `padLeft` 조립은 그대로 남아, 같은 유효기간이 **한 화면 안에서** 두 모양으로 보였다(중복 등록 다이얼로그는 점, 유효기간 필드는 하이픈). v2.4가 증거로 든 `test/features/scan/gifticon_form_input_test.dart`도 그 죽은 폼을 검증하던 것이라 #127에서 함께 삭제됐다 — **죽은 코드를 고치고 통일됐다고 기록한 것**이 이 오류의 형태다. 실제 통일은 이 버전에서 이뤄졌고(`scan_page.dart`가 정본 `formatYmdDot` 소비), 회귀 테스트를 `test/features/scan/scan_form_screen_test.dart`에 붙였다(뮤테이션으로 무는 것 확인 — 기대값은 `DateTime.now()`가 아니라 **라벨이 실제로 읽는 폼 상태**에서 만든다. 자정에 날짜가 넘어가면 코드가 아니라 달력 때문에 빨개지는데, 이 저장소는 같은 유형으로 2026-08-27T00:31Z CI가 실제로 터진 이력이 있다). v2.4 행은 당시 기록으로 보존한다. | scan(표시 문자열 변경 — 이번에 실제 반영) |
| v3.11 | 2026-08-30 | **인라인 에러 배너 승격 (non-breaking, 이동 + 개명).** `ShareErrorBanner`(`lib/features/share/widgets/share_error_banner.dart`)를 `InlineErrorBanner`(`lib/shared/widgets/inline_error_banner.dart`)로 옮기고 개명했다 — 공유 도메인 전용이 아니게 되어 `Share` 접두가 사실과 어긋난다. **이동·개명만이며 렌더링·동작은 불변**이다(옛 파일에 개명·import 치환만 적용한 결과와 `library;` 이후 전체가 바이트 단위로 일치). `tool/check_ssot.sh`에 `InlineErrorBanner`를 등록했다 — 위젯이라서가 아니라 **행위 계약을 실어 나르기 때문**이다(자동 재시도 금지 = retry storm 회피 #13, 내부 예외 메시지 비노출, null 재시도의 안내 접미). 사본은 이 셋을 컴파일러에 안 잡힌 채 떨어뜨린다. ⚠️ **승격 시점의 실상**: PR #137이 scan에 그룹 선택 타일을 넣어 **두 번째 소비 지점이 확정**됐을 뿐, 배선은 아직이며 **현재 실소비자는 share 하나**다(`lib/features/scan`에 참조 0건). 즉 CLAUDE.md 승격 워크플로 ③이 말하는 "실제 두 번째 소비자"가 이미 생긴 상태는 아니고, 지점 확정 후 사본이 생기는 창을 SSOT 가드로 닫으려고 앞당긴 승격이다 — 문서가 규칙 충족을 단정하지 않도록 이 사실을 계약 문서·헤더 doc 양쪽에 적었다. **[에이전트 리뷰 반영]** 🟠 헤더 doc이 main·auth에 **틀린 배선을 처방**하고 있었다 — "재시도 경로가 없는 지점이라 `onRetry: null`"이라 단정했지만 두 지점(`lib/features/main/main_page.dart`·`lib/app/auth_gate.dart`) 다 에러 원천이 `sessionUserProvider`이고 `retrySessionIfFailed`(`lib/shared/providers/session_provider.dart`)가 정확히 그 대상이다. 그대로 따랐으면 제자리 복구가 가능한 지점에서 사용자가 버튼 없는 배너와 "앱을 다시 열어 주세요"를 봤을 것이고, `lib/shared`의 헤더 doc은 CODEOWNERS가 걸린 계약 문서라 다음 담당자가 그대로 배선한다. `onRetry: () => retrySessionIfFailed(ref)`로 정정했다(배선 자체는 각 담당의 별도 PR — 이번엔 처방 문구만). `onRetry == null` 분기를 유지하는 근거도 "지금 그런 사이트가 남아 있어서"에서 **"앞으로 생길 때의 안전망"**으로 바로잡았다(전제가 거짓이었다). 🟠 **이 행의 버전 번호를 v3.2로 새길 뻔했고 그건 알림 provider 승격에 이미 배정된 번호다** — v3.8 행이 **똑같은 실수를 똑같은 번호로** 기록해 둔 자리에서 재발했다. 이번에는 표를 실제로 조회해(`grep -oE "^\| v[0-9.]+ "` → 마지막 행 v3.10, v3.11 미사용) 번호를 정했고, 7행 계약 버전 줄이 v3.9에 멈춰 있던 것(이 파일 9-10행이 "표 마지막 행과 반드시 함께 갱신"이라 못박은 규칙 위반)도 함께 맞췄다. 🟡 헤더 doc의 "옛 이름 grep 0건"이 **저장소 전체 기준으로는 거짓**이었다(`_workspace/qa_report.md` 4곳). 매트릭스는 범위를 `lib`·`test`로 한정했는데 헤더만 넓혔던 것 — 범위를 한정하고, 그 4곳은 날짜 박힌 과거 QA 스냅샷이라 **개명 금지**임을 함께 적었다(다음 사람이 누락으로 읽지 않게). 🟡 복구 안내 문구를 지키는 단언이 상수를 그대로 보간해 **자기참조**였다(문구를 바꿔도 기대값이 함께 바뀌어 통과 — 뮤테이션으로 28/28 green 확인). 리터럴로 고정하고 상수 값 자체도 따로 못박았다. 🔵 테스트 파일 import가 이 PR에서 정렬이 깨진 것(lib 소비 6곳은 새 경로의 알파벳 위치로 옮겼는데 테스트만 옛 자리에 치환)도 함께 고쳤다 — `analysis_options.yaml`에 `directives_ordering`이 없어 `flutter analyze`가 잡지 않는다. 그 밖에 헤더 doc이 `scanTargetGroupsProvider`를 **단수로** 지목하던 것을 경로로 명시했다 — 같은 이름이 `lib/features/scan/state/scan_target_group_state.dart`(`Provider.autoDispose`, 화면이 실제로 읽는 쪽)와 `lib/features/share/state/share_providers.dart`(`Provider`, 참조 0건)에 **서로 다른 수명으로** 두 번 선언돼 있어 다음 배선 담당이 어느 쪽인지 알 수 없었다(두 선언 정리는 scan·share 담당의 별도 작업). 테스트 637 유지 · analyze 무이슈. | share(경로·이름 변경 — 소비 전환 완료), scan·main·auth(후속 배선 대상) |
| v3.12 | 2026-09-01 | **`InviteExpiredException`이 자격증명 종류를 싣는다 (⚠️ breaking — 생성자에 `required isCode` 추가).** 만료 안내는 링크와 코드를 갈라야 한다 — 코드는 5분이라 만료가 정상 경로라 "새 코드를 요청"이고, 링크는 24시간이라 "링크 재발급을 요청"이다. **다음 행동이 다르므로 뭉뚱그리면 안내가 사용자를 틀린 곳으로 보낸다.** 그런데 화면이 `isWellFormedInviteCode(token)`으로 **모양**을 보고 갈라, v3.0(128비트) 이전의 6자리 **링크 토큰**을 코드로 오분류했다 — 그 값들은 아직 살아 있다(팀 공용 시드 `482913`, 데모 시드, 레거시 Firestore 문서). 그 링크가 만료된 사람은 "새 코드를 요청하세요"를 보고 코드를 받아 오지만 **만료된 링크는 그대로다.** v3.9의 CodeRabbit 지적이 문구를 둘로 나눴고 PR #152가 확인 모드(딥링크 진입)를 `!_isConfirmMode`로 닫았지만, **입력 모드는 원리상 고칠 수 없었다** — 손으로 붙여넣은 6자리에는 화면이 볼 판별 신호가 없다. (1) **판정을 한 곳으로 모은 것이 이 변경의 전부다.** 두 구현은 코드 조회 문서를 먼저 보고 없으면 링크 토큰으로 **폴백**하므로 *이미* 종류를 확정하고 있고, 그것이 만료 창을 고른 근거이기도 하다. 그 확정된 사실을 `isCode`로 실어 보내면 화면이 같은 판정을 **더 나쁜 근거로 되풀이할** 이유가 사라진다(같은 입력에 저장소는 '링크', 화면은 '코드'라고 답하던 어긋남). (2) `isCode`에 **기본값을 두지 않았다** — 두면 판정을 아는 구현체가 그것을 싣지 않고 넘어가는 길이 열리고, 그 침묵은 화면에서 "링크"라는 단정으로 나타난다. `required`라서 호출부 누락은 컴파일러가 막는다. (3) 계약 dartdoc에 구현체 의무를 명시했다 — 폴백 판정을 그대로 실을 것, **만료 판정에 쓴 창과 같은 값**일 것. (4) 화면은 `!_isConfirmMode` 우회와 모양 검사를 함께 걷어냈고, 세 갈래를 튜플 대신 객체 패턴으로 풀어 `(false, true)`(만료가 아닌데 코드)라는 있을 수 없는 조합을 표현 불가능하게 했다. **[에이전트 리뷰 반영]** 🟡 **firebase 구현의 `isCode = false` 한 줄에 커버리지가 0이었다** — 지워도 662건이 전부 green이었다(리뷰어가 뮤테이션으로 증명). `flutter test`는 in-memory만 돌리므로 실서비스 백엔드에서만 이 PR이 고친 오분류가 되살아나는 형태다(반복 패턴 #1). 같은 이유로 만들어져 있던 소스 검사 테스트(`firebase_share_write_scope_test` — "호출부가 `DocumentSnapshot`을 받아 Firestore 없이 실행할 수 없다")에 케이스 6건을 붙였고, 그 뮤테이션이 실제로 잡히는 것을 확인했다. 창은 첫 `}`가 아니라 **중괄호 깊이**로 잡는다 — 블록 안에 보간(`${…}`)이 하나만 생겨도 첫 `}`에서 잘려 동작이 그대로인데 실패한다. 무는 구간은 **네 라운드에 걸쳐** 넓혔고 매 라운드 리뷰가 새 구멍을 실측으로 찾아냈다: ①처음엔 두 줄뿐이라 1차 조회 컬렉션 뒤집기와 던지기 직전 모양 재대입이 통과 ②1차 조회 줄과 폴백 앵커 **사이 두 줄**(초기값·폴백 진입 조건)이 비어 세 변형이 통과 ③마지막은 판정을 **읽는** 자리만 보고 **만드는** 자리(`codeShaped` 정의)를 안 봐서, 그 한 줄을 뒤집으면 모든 초대코드가 죽는데도 668건이 전부 green이었다 — 하필 in-memory의 같은 줄은 행위 테스트 6건이 물고 있어, 이 그룹이 메우려던 두 구현 간 비대칭이 정작 **입력 쪽에** 남아 있었다. 교훈은 하나다 — **소스 검사는 무는 구간을 뮤테이션으로 재지 않으면 커버리지를 계속 과대평가한다.** 주석이 '이 줄을 못박는다'고 적어 두는 것은 근거가 아니다(실제로 두 번, 검증하지 않는 것을 주장하는 주석이 그 구멍을 가렸다). 🟡 **폐기된 전제가 하필 판정 함수 자신의 dartdoc에 남아 있었다** — `invite_code.dart`가 "둘은 모양으로 구분된다"고 단언하는데, 같은 저장소의 `invite_code_test.dart`는 "모양만으로는 구분되지 않는다"를 케이스 제목으로 못박고 있었다(자기모순). IDE 호버가 보여주는 것이 그 dartdoc이라, 같은 전제로 세 번째 사고가 날 자리였다(v3.9에서 정상 링크를 통째로 막았고, PR #152에서 화면이 반복했다). 🟡 이 문서(계약 SSOT) 자신이 갱신되지 않아 `requestToJoin` 행이 방금 반박된 전제를 계약으로 서술하고 있었다 — 직전 8개 계약 커밋 중 7개가 이 문서를 함께 갱신했다. 테스트 **668** · analyze 무이슈 · 규칙 검증 해당 없음(`firestore.rules` 무변경). | share(만료 안내), 계약(예외 시그니처 — 호출부 전부 갱신됨) |
| v3.13 | 2026-09-07 | **중복 참여 요청을 조용한 반환에서 예외로 (⚠️ breaking — `requestToJoin`의 성공 경로 하나가 사라졌다).** 두 상황이 화면에서 뭉뚱그려져 있었다. **이미 대기 중인 요청**은 실패조차 아니었다 — 저장소가 기존 요청을 조용히 반환해서 화면이 "참여 요청을 보냈어요"를 다시 띄웠고, 승인을 기다리다 답답해 다시 눌러 본 사람에게 **새로 보낸 것 같은 착시**를 줬다(그래서 계속 다시 누른다). **이미 멤버**는 평범한 `StateError`라 화면이 "링크나 코드가 잘못됐을 수 있어요"로 안내했다 — 이 실패에는 다음 행동이 없는데(코드를 다시 받아 와도 같은 답이다) **고칠 수 없는 것을 고치게** 만든다. (1) 두 실패를 `AlreadyGroupMemberException`·`JoinRequestAlreadyPendingException`으로 좁혔다 — 계약이 문자열 판정을 금하므로(`InviteExpiredException`의 "문자열로 구별하지 않는 이유") 화면이 갈라 말하려면 타입이 필요하다. 둘 다 `StateError`를 상속해 기존 `on StateError`·`catch (e)`는 그대로 잡는다. (2) **저장소의 부작용은 바뀌지 않았다** — 대기 중이면 여전히 아무것도 쓰지 않는다(방장에게 요청이 쌓이지 않는다는 멱등의 목적 그대로). 바뀐 것은 그 사실을 호출부가 안다는 것뿐이다. 판정을 화면이 대신할 방법은 없다 — 요청자는 아직 비멤버라 그룹도 대기 목록도 읽을 수 없다(보안 규칙). (3) **결정이 끝난 요청(거절·강퇴 후)은 그대로 되살아나 정상 반환이다** — 거기까지 예외로 닫으면 오거절의 복구 경로가 사라진다. 계약 스위트가 두 분기를 **두 구현 모두에서** 갈라 고정한다. (4) 화면은 실패 안내를 하단 스낵바에서 `AlertDialog`로 옮겼다 — 스낵바는 시트 뒤편 아래쪽에 잠깐 떴다 **저절로** 사라져서 방금 버튼을 누른 사람이 놓치기 쉽고, 키보드가 올라온 입력 모드에서는 가려지기까지 한다. 예외→문구 매핑은 `_JoinFailure` 한 곳에 모았다(문구를 고르는 일과 코드 입력 전환을 여는 일이 서로 다른 자리에서 같은 예외를 각자 분류하면, 한쪽만 갱신되는 날 "고칠 것이 없다고 말하면서 고치는 입력란을 여는" 화면이 된다). **[에이전트 리뷰 반영]** 🟠 **스낵바를 걷어내면서 `await` 이전의 사전 캡처를 함께 지웠다** — 시트는 응답을 기다리는 동안 내려갈 수 있고(`showModalBottomSheet`의 기본값이 `isDismissible`·`enableDrag`, firebase 경로는 쓰기 상한이 15초라 창이 길다), 그때 `_showFailureDialog`의 mounted 가드가 **실패를 아무 안내 없이 삼켰다**(리뷰어가 프로브로 재현: 이 커밋 `AlertDialog` 0건·`SnackBar` 0건, develop `SnackBar` 1건). `reportHandledFailure`도 `ref`가 죽어 진단조차 남지 않는다 — 같은 catch 블록의 주석이 "실패 원인과 무관하게 **항상** 결과를 알려준다"고 적어 둔 불변식을 주석만 남긴 채 깬 것이다. 폴백을 스낵바로 두는 대신 **루트 내비게이터를 `await` 이전에 캡처**해 팝업을 그대로 띄운다 — 팝업은 어차피 루트에 올라가므로(`showDialog`의 `useRootNavigator` 기본값) 시트가 사라져도 자리가 남고, 스낵바로 떨어뜨리면 하필 이 변경이 걷어낸 형태로 되돌아간다. 시트를 내린 뒤 실패시키는 테스트로 고정했다. 🟡 계약 파일이 자기모순이었다 — `joinGroup` 삭제 대응표(옛 이름을 찾아 오는 사람이 **처음 읽는 자리**)만 "지금은 `StateError`로 거부"에 멈춰 있었다. 🟡 firebase 쓰기 상한(15초)의 **안전성 근거**가 이 커밋이 지운 동작을 인용했다("다음 요청에서 멱등 분기가 그대로 돌려준다") — 상한을 손보러 오는 사람이 반증된 전제를 근거로 읽는다. 🟡 이 문서가 갱신되지 않았다(v3.12 행이 같은 지적을 기록해 둔 자리에서 **한 PR 만에 재발**). 🟡 **2라운드** — 팝업은 사전 캡처로 살렸는데 같은 catch의 **진단은 그대로 사라졌다**(커밋 메시지가 결함으로 적어 둔 절반이 안 고쳐졌다). `reportHandledFailure`는 `WidgetRef`를 쓰므로 시트가 죽으면 `ref.read`가 던지고 자기 `catch (_)`에 삼켜진다 — 사용자 안내는 그 설계 덕에 지켜지지만 로그만 조용히 빈다. 하필 그 표본이 원격 진단에 가장 필요한 것이라(화면을 떠난 뒤의 백엔드 실패) 빠지면 로그가 "시트를 안 닫은 실패"로 편향된다. `lib/shared/diagnostics/`에 `reportHandledFailureTo(reporter, …)` 형제를 두고(기존 함수는 이제 그것을 감싼다 — 진입점 계약 유지) 시트 셋 **모두**에서 리포터를 `await` 이전에 잡는다. 하나만 고치면 같은 catch 안에서 안내는 살고 진단은 죽는 비대칭이 시트마다 갈린다. 신규 공유 타입 둘은 `tool/check_ssot.sh`에 등록했고 임시 재선언으로 실제 검출을 확인했다. 테스트 **769** · analyze 무이슈 · 규칙 검증 해당 없음(`firestore.rules` 무변경). ⚠️ 이 계량은 #169 머지 뒤 develop에 **리베이스한 트리**에서 다시 잰 값이다(리베이스 전 베이스 `86a264e` 기준으로는 751이었고, 차이는 #169가 들고 온 테스트다) — 리베이스한 PR은 계량을 다시 잰다. | share(참여 요청 실패 안내), 계약(반환 의미 변경 — 호출부 전부 갱신됨) |
| v3.14 | 2026-09-08 | **만료된 기프티콘 유효기간 연장 (non-breaking, 순수 추가).** 만료돼서 못 쓰게 된 기프티콘을 브랜드사가 연장해 줬을 때 그 결과를 앱에 반영할 경로가 없었다. (1) `GifticonRepository.extendExpiry(id, newExpiryDate)` 신설 — 만료일을 **뒤로만** 옮기고(`isLaterExpiryDate` 신설 — 달력 일 단위 판정. `isAfter`로 재면 같은 날 시각만 뒤인 값이 통과해 "연장했는데 D-day가 그대로"가 된다), 저장된 `expired`를 `available`로 되돌린다. **그 되돌림을 전이 표에는 넣지 않았다** — 표에 넣으면 `updateStatus(id, available)`로도 되살릴 수 있게 되어 만료일은 그대로인 채 상태만 살아나는 문서가 생긴다. 표는 "상태만 바꾸는 API"의 규칙이고 연장은 둘을 함께 옮기는 별도 계약이다. `used`는 이 예외에서도 되돌아오지 않는다. (2) `ShareRepository.extendSharedExpiry(id, newExpiryDate)` 신설 — **`SharedGifticon.expiryDate`가 원본의 스냅샷**이라 한쪽만 옮기면 그룹 화면과 개인 목록이 같은 기프티콘을 두고 다른 만료일을 말하고, 그 어긋남은 어느 화면에도 보이지 않는다. 그래서 둘을 한 단위로 처리하고 순서를 **원본 → 스냅샷**으로 못박았다(원본이 실패하면 아무 일도 없었던 것이 되고 화면은 실패를 그대로 안내한다. 반대 순서면 "그룹에는 늘어난 날짜, 내 목록에는 옛 날짜"가 조용히 남는다). 원본 갱신은 트랜잭션 밖이다 — 주입된 `GifticonRepository`가 같은 백엔드라는 보장이 계약에 없다. 그래서 원본만 성공하는 창이 남고, 복구는 **같은 연장을 다시 하는 것**이다(원본이 이미 그 날짜 이후면 건너뛰고 스냅샷만 따라붙어 둘이 만난다). `markUsed`의 원본 동기화와 달리 실패를 삼키지 않는 이유: 그쪽 행위자는 아무 멤버나 될 수 있어 남의 원본에 권한이 없지만, 연장의 행위자는 가드로 **공유자 = 소유자**로 좁혀져 권한이 항상 있다. (3) **"등록한 본인만 연장"의 강제는 두 겹이다** — 계약 가드(`sharedByUserId != 행위자`면 `StateError`)와 `firestore.rules`. 규칙은 `sharedGifticons`의 필드를 세 등급으로 나눴다 — **신원**(`groupId`·`gifticonId`·`sharedByUserId`)은 불변, **원본 스냅샷**(`expiryDate`·`brand`·`productName`·`barcode`)은 공유자만, **사용 흐름**(`status`·잠금·찜)은 멤버 누구나(그것이 공유의 목적이라 함께 좁히면 안 된다). ⚠️ **신원을 못박지 않으면 스냅샷 제한이 성립하지 않는다** — `sharedByUserId`가 스스로 그 판정 근거인데 가변이면, 멤버가 먼저 자기 uid로 바꿔 놓고(그 요청은 만료일을 안 건드리니 통과) 그다음 연장하면 된다. 초안이 정확히 그랬고, 에이전트 리뷰가 에뮬레이터로 두 요청을 재현했다(🔴). `delete`도 같은 이유로 공유자로 좁혔다 — 지운 자리에 **같은 문서 id로 다시 만들면**(`create`는 재생성인지 원리상 알 수 없다) 제한을 건너뛴다. **정리 권한은 방장에게 남겼다** — 문서 모양(`sharedByUserId` 유무)으로 예외를 두면 그것이 곧 우회로다(초안이 그랬다: 우회의 목적은 만료일이 아니라 **주인이 되는 것**이고, `create`에 필드 검증이 없어 지운 자리에 자기를 sharer로 재생성하면 그만이다 — 에뮬레이터 실측 삭제 200 → 재생성 200 → 연장 200). 반대 방향도 같은 조항이 닫는다: 공유자가 그룹을 떠나면 공유자도 비멤버라 못 지우고 남은 멤버도 못 지운다. **`create`도 같은 이유로 좁혔다**(CodeRabbit 지적) — `update`에서 신원을 못박아도 생성이 열려 있으면 **남의 이름으로 새 항목을 만들면 그만**이라 ① 등급이 종이 규칙이 된다. 이제 `sharedByUserId == uid()`와 **원본 소유**(`gifticons/{gifticonId}.ownerId == uid()`)를 요구하고 필드도 allowlist로 좁힌다. 그 소유 검사가 delete → create **가로채기**도 함께 닫는다(재생성은 자기 소유 원본으로만 된다). 잔여: 방장은 남의 공유 항목을 **지울** 수는 있다 — 지운 사실을 남기려면 별도 기록이 필요하다. ⚠️ **그 방장 조항은 아직 소비자가 없다** — 유일한 자동 정리 경로인 `_cascadeDeleteGroupShares`가 그룹 문서를 지운 **뒤** 돌아 `isGroupMember`부터 막힌다(실측 403). 그래서 그룹을 지우면 공유 항목·잠금이 통째로 남고, `batch.commit()`의 PERMISSION_DENIED가 `deleteGroup`의 try 밖에서 전파돼 **그룹은 지워졌는데 화면은 삭제 실패로 안내**한다. 선재 결함이고 순서 교정은 복원 설계(`_restoreInviteCredentials` 같은 대응물이 없다)를 요구하므로 별건으로 세웠다. `.get(…, '')` 폴백은 지금 무언가를 막고 있는 것이 아니라(`||` 단락 평가로 그 항에 닿지 않는다 — 직접 접근으로 바꿔도 전건 통과) 그 검사를 `||` 밖으로 옮기는 리팩터가 레거시 문서의 **모든 쓰기**를 막지 않게 하는 대비다. 검증 케이스 34건 추가(픽스처·정리 포함, 총 130건). (4) `GroupNotificationType.expiryExtended` 추가 — 만료라 못 쓴다고 판단했던 멤버가 다시 쓸 수 있게 됐음을 알 경로가 그것뿐이다(목록의 만료 표시는 다시 볼 때까지 안 바뀐다). `notification_center_page`의 exhaustive switch가 아이콘 누락을 컴파일 타임에 잡아 함께 채웠다. (5) 테스트 — `gifticon_repository_contract_suite`를 **신설**해 `extendExpiry`를 in-memory·firebase(fake) 두 구현에 같은 기대로 돌리고(반복 패턴 #1: `flutter test`가 in-memory만 돌려 두 구현이 갈라져도 green), 공유 스위트에는 연장 계약을 더했다(스냅샷·원본 동반 이동, 비공유자 거부 시 **양쪽 다** 불변, 원본 부재·원본이 더 뒤인 경우, 그리고 **연장 도중 사용 완료**). 마지막 케이스가 에이전트 리뷰의 🟠을 고정한다 — in-memory가 `await` 뒤에 자리만 다시 찾고 **판정을 다시 하지 않아**, 같은 입력에 in-memory는 성공하고 firebase는 `StateError`였다(그 사이 사용된 항목의 만료일이 옮겨지고 "기간 연장" 알림까지 게시됐다). 테스트 **809** · analyze 무이슈 · 규칙 검증 **130/130**. | main·share(연장 UI는 후속 PR — 이번 슬라이스는 계약·저장소·규칙까지) |
| v3.15 | 2026-09-09 | **연장 UI 두 화면 + 그에 딸린 계약 승격 (non-breaking, 순수 추가).** v3.14가 앉힌 계약에 화면을 붙이면서, 두 화면이 함께 쓰는 것 셋이 페이지 밖으로 나왔다. **① `expiry_date_range.dart`(`expirySelectableFrom`/`expirySelectableTo`) — `lib/features/scan/util` → `lib/shared/util`.** 유효기간 피커의 경계다. 승격 없이는 연장 다이얼로그가 scan을 향해 import해야 하는데 이 저장소에 교차 페이지 import 선례가 없고, 다음 사람은 사본을 뜬다 — 이 저장소가 `isExpiredByDate`를 승격한 이유가 정확히 그것이었다(`lib/features`에 네 벌, 그중 하나가 시분초를 절삭해 화면과 통계가 다른 답을 냄). **② `extend_expiry_dialog.dart` — `lib/features/main/widgets` → `lib/shared/widgets`.** 소비자가 둘이 됐다(개인 상세·공유 상세). 사본을 뒀다면 하한 계산의 DST 결함(v3.14 후속에서 잡힌 것)이 한쪽에만 고쳐졌을 것이다. **③ `detailActionButtonStyle()` — `lib/shared/widgets/gifticon_detail_widgets.dart`에 신설.** 같은 버튼 스타일 블록이 세 벌로 흩어져 있었고(개인 상세 2 + 공유 상세 1) 연장 버튼이 네 번째를 만들 참이었다(CodeRabbit 지적). ⚠️ 이 스타일은 `theme.textTheme.labelLarge`를 **소비하지 않는다** — 앱의 `TextTheme`이 그 슬롯을 정의하지 않아 Material 기본값(`height 1.43`·`letterSpacing 0.1`)이 딸려 들어오고 기존 버튼이 52 → 56px로 자란다(측정값). 버튼 타이포 토큰화는 앱 전역 변경이라 별건. **`tool/check_ssot.sh`**: ①의 두 상수를 가드에 등록하면서 `const`만 검사하던 스캔을 `const|final`로 넓혔다 — 그 둘은 `DateTime`이라 `const`가 될 수 없어, 이름만 넣으면 가드가 통과만 시킨다(등록 전 실측). 테스트 **837**(신규 8) · analyze 무이슈. | main(연장 버튼·경로 분기), share(연장 버튼 — 공유자만), scan(import 경로만 변경) |
| v3.16 | 2026-09-17 | **`shareGifticon`이 원본의 현재 상태를 검사한다 (non-breaking — 새 거부 사유는 `StateError`라 기존 `on StateError`·`catch (e)`가 그대로 받는다).** 저장소가 막는 축이 그룹 없음·비멤버·이중 공유 셋뿐이라, 공유 확인 팝업이 떠 있는 사이 다른 기기가 원본을 사용 완료로 옮겨도 공유 레코드가 만들어졌다 — 멤버는 매장에서야 안다(#169·#171이 미룬 항목). (1) 호출자가 넘긴 **스냅샷**이 아니라 `GifticonRepository.getGifticonById`로 **현재** 원본을 다시 읽어 `available`이 아니면 거부한다. 조회는 계약 인터페이스로, Firebase 구현에서는 트랜잭션 **밖**에서 한다 — 원본은 공유 저장소의 것이 아니고, 원본 동기화(`markUsed`·`extendSharedExpiry`)도 같은 계층 규약을 써 왔다. 남는 창은 한 왕복(밀리초)이다. (2) **레코드의 표시 필드(브랜드·상품명·만료일·바코드)도 그 원본으로 만든다** — 팝업 사이 연장된 만료일을 옛 스냅샷으로 실으면 v3.14가 막으려던 스냅샷·원본 어긋남이 공유 시점에 생긴다(에이전트 리뷰가 두 백엔드에서 재현). (3) 원본을 **못 찾으면 건너뛴다**(데모 시드의 가짜 id — 원본 동기화와 같은 규약). 계약에 기프티콘 삭제 API가 없어 "다른 기기가 원본을 지웠다"는 앱 경로가 없고, 규칙 있는 백엔드에서는 없는 원본의 공유가 원래(가드 도입 전 기준) `sharedGifticons` 생성 규칙의 `ownsGifticon`에 거부됐고(규칙 검증 "없는 원본을 붙여 공유 → 차단"), 가드가 생긴 지금은 그보다 먼저 가드의 읽기가 `gifticons` 읽기 규칙에 막힌다(예외는 `FirebaseException`). ⚠️ 규칙 백엔드에서 **없는** 원본의 `getGifticonById`는 `null`이 아니라 403이다(`gifticons` 읽기 규칙이 `resource.data` 참조 — 계약 "없으면 null" 위반) — 도달 경로가 없어 고치지 않고 구현 주석에 가정을 명시했다(삭제 API가 생기면 거기서부터 다시 본다). 보안 규칙 계층에는 원본 상태 조항을 두지 않았다(행위자가 자기 기프티콘을 공유하는 본인). (4) 계약 스위트에 `runShareSourceStatusContract`를 신설해 두 구현에 같은 기대를 돌린다. 연장 계약의 `shareOne`이 "저장 상태가 `expired`인 원본을 공유"하는 지름길을 써서 새 가드에 막혔고, `available`로 공유한 뒤 전이시키도록 바꿨다. ⚠️ 반대 방향 경합(상세 화면의 '사용 완료' 확인 팝업 사이 다른 기기가 공유하면 원본만 `used`)은 이 계약 밖(화면의 재확인 누락)이라 `gifticon_detail_page` 후속 PR에서 다룬다. 테스트 **879** · analyze 무이슈 · 규칙 검증 해당 없음(`firestore.rules` 무변경). | share(공유 시트 — 거부 시 기존 안내), scan(방금 저장한 원본이라 영향 없음) |
