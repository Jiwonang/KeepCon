...
| `nowProvider` (`Provider<DateTime>`) — **SSOT**. 날짜 파생 계산이 공유하는 단일 시각. 소비자가 각자 `DateTime.now()`를 읽으면 서로 다른 "오늘"로 판정한다. 값은 셸이 앱 복귀 시 무효화해 갱신(포그라운드 자정은 미해결). 날짜 파생 소비자는 이것만 본다 — `DateTime.now()` 직접 읽기 금지 | provider | `lib/shared/providers/now_provider.dart` | 3.3 |
