...
| v3.15 | 2026-09-10 | **연장 UI 두 화면 + 그에 딸린 계약 승격 (non-breaking, 순수 추가).** v3.14가 앉힌 계약에 화면을 붙이면서, 두 화면이 함께 쓰는 것 셋이 페이지 밖으로 나왔다. **① `expiry_date_range.dart`(`expirySelectableFrom`/`expirySelectableTo`) — `lib/features/scan/util` → `lib/shared/util`.** 유효기간 피커의 경계다. 승격 없이는 연장 다이얼로그가 scan을 향해 import해야 하는데 이 저장소에 교차 페이지 import 선례가 없고, 다음 사람은 사본을 뜬다 — 이 저장소가 `isExpiredByDate`를 승격한 이유가 정확히 그것이었다(`lib/features`에 네 벌, 그중 하나가 시분초를 절삭해 화면과 통계가 다른 답을 냄). **② `extend_expiry_dialog.dart` — `lib/features/main/widgets` → `lib/shared/widgets`.** 소비자가 둘이 됐다(개인 상세·공유 상세). 사본을 뒀다면 하한 계산의 DST 결함(v3.14 후속에서 잡힌 것)이 한쪽에만 고쳐졌을 것이다. **③ `detailActionButtonStyle()` — `lib/shared/widgets/gifticon_detail_widgets.dart`에 신설.** 같은 버튼 스타일 블록이 세 벌로 흩어져 있었고(개인 상세 2 + 공유 상세 1) 연장 버튼이 네 번째를 만들 참이었다(CodeRabbit 지적). ⚠️ 이 스타일은 `theme.textTheme.labelLarge`를 **소비하지 않는다** — 앱의 `TextTheme`이 그 슬롯을 정의하지 않아 Material 기본값(`height 1.43`·`letterSpacing 0.1`)이 딸려 들어오고 기존 버튼이 52 → 56px로 자란다(측정값). 버튼 타이포 토큰화는 앱 전역 변경이라 별건. **`tool/check_ssot.sh`**: ①의 두 상수를 가드에 등록하면서 `const`만 검사하던 스캔을 `const|final`로 넓혔다 — 그 둘은 `DateTime`이라 `const`가 될 수 없어, 이름만 넣으면 가드가 통과만 시킨다(등록 전 실측). 테스트 **837**(신규 8) · analyze 무이슈. | main(연장 버튼·경로 분기), share(연장 버튼 — 공유자만), scan(import 경로만 변경) |
