...
작성일: 2026-07-06 · 최종 갱신: 2026-09-01 · 계약 버전: **v3.12** (⚠️ breaking — `InviteExpiredException`이 자격증명 종류를 싣는다)
...
| `ShareRepository.requestToJoin(credential)` → `Future<JoinRequest>` — `credential`은 **링크 토큰 또는 6자리 초대코드**. 모양(`isWellFormedInviteCode`)은 **첫 조회 대상의 힌트일 뿐**이고(v3.0 이전 6자리 링크 토큰이 살아 있다) 정본은 **어느 조회 문서에서 찾았는지**다. 만료는 **사용한 자격증명 기준** | method | 〃 | 3.12 |
