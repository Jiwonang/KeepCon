...
# 실기기(Android) — 기기를 연결하고 `-d chrome` 만 빼면 된다.
# 개발자 옵션 > USB 디버깅을 먼저 켜야 한다.
...
> *Visual Studio not installed* 는 **무시하세요.** Windows **데스크톱** 앱 빌드용이고 KeepCon은 web·android만 구성돼 있습니다. 이름이 비슷할 뿐 VS Code와는 무관합니다.
...
- **푸시 전 로컬 검증 — [`tool/verify.sh`](tool/verify.sh) (명령 하나):** `dart format`·`analyze`·`test`·`SSOT guard`·`Markdown lint`를 항상 돌리고, **규칙 계층 입력이 바뀌었으면 에뮬레이터 규칙 검증까지 자동으로** 붙입니다. 예전에는 규칙 검증이 조건부라 매번 사람이 판단했는데 그 판단이 실제로 세 번 연속 실패했습니다 — 그래서 판단을 없앴습니다.
  ...
  - 감시 대상은 `firestore.rules`뿐 아니라 `tool/verify_firestore_rules.sh`·`tool/lib_rules_state.sh`·`firebase.json`도 포함합니다 — CI의 `Firestore rules` 잡은 **모든 PR에서** 돌기 때문에, 로컬 트리거가 좁으면 CI에서 처음 빨개집니다.
  - 에뮬레이터 재사용은 **떠 있다는 것만으로 결정하지 않습니다.** 떠 있는 에뮬레이터가 실제로 적재한 규칙 본문을 조회해(`GET /emulator/v1/projects/{project}:securityRules`) 로컬 `firestore.rules`와 대조하고, **일치할 때만** 재사용합니다. 다르거나 확인할 수 없으면(또는 절반만 떠 있으면) **다른 포트에 전용 인스턴스**를 띄워 검증합니다 — 개발용 에뮬레이터는 죽이지 않습니다(`.emulator-local/` 개인 데이터는 정상 종료에만 저장되므로 강제 종료하면 그 세션이 통째로 날아갑니다). 아무것도 떠 있지 않으면 기본 포트로 직접 띄웁니다(새 인스턴스라 스테일 문제가 없습니다).
    - 낡은 규칙을 문 에뮬레이터에 붙으면 검증이 **조용히 무의미해집니다.** 2026-08-26 실측 — 다른 체크아웃에서 띄운 에뮬레이터에 붙어 멀쩡한 규칙이 대량 실패로 보였고, 같은 스크립트를 새 인스턴스에 붙이니 전부 통과였습니다. **반대 방향이 더 위험합니다**: 규칙을 느슨하게 바꾼 변경이 옛 엄격한 규칙 위에서 green으로 통과합니다.
    - 같은 가드가 [`tool/verify_firestore_rules.sh`](tool/verify_firestore_rules.sh) **자신**에도 들어 있습니다([`tool/lib_rules_state.sh`](tool/lib_rules_state.sh) 공용) — 아래 단독 실행 안내와 `tool/deploy_rules.sh`의 배포 직후 검증이 그 스크립트를 직접 부르기 때문입니다. 단독 실행에는 전용 인스턴스라는 대피로가 없으므로 낡은 규칙이면 **실패로 끝냅니다.**
    - 검증용으로 띄운 에뮬레이터는 끝나면 **거둡니다**(바인드 수준에서 우리가 잡은 포트 + 부모가 죽은 프로세스, 두 조건을 모두 만족할 때만 — 개발용 에뮬레이터는 어느 쪽도 아닙니다). `firebase emulators:exec`는 종료 코드 0으로 끝난 뒤에도 Firestore 에뮬레이터(java) 프로세스를 남기는데(firebase-tools [#1367](https://github.com/firebase/firebase-tools/issues/1367)·[#2477](https://github.com/firebase/firebase-tools/issues/2477)·[#3179](https://github.com/firebase/firebase-tools/issues/3179)), 그대로 두면 그 포트가 영구히 막혀 다음 실행부터 자리가 하나씩 줄어듭니다.
...
- **4층 · AI 리뷰 — CodeRabbit (두 번째 의견, ⚠️ 수동 트리거 · PR 후):** 협업 규칙 준수·버그를 리뷰합니다. 설정은 [`.coderabbit.yaml`](.coderabbit.yaml)에 한국어로 정의(별도 API 키·워크플로 불필요). **단, 설정 파일만으로는 동작하지 않습니다** — CodeRabbit은 **GitHub App**이라 저장소에 설치·접근 허용되어야 합니다.
  - **⚠️ 이 층은 자주 빕니다 — 그래서 예비입니다.** ①**PR을 열어도 자동으로 리뷰하지 않습니다**(스타 10개 미만 저장소는 수동 트리거로 전환됨) ②트리거해도 시간 창 **할당량**에 걸려 `Review rate limited`로 튕깁니다(PR #90이 써서 #92가 두 번 튕겼습니다). 리뷰가 없는 PR이 생기지 않도록 **2층을 기본으로 두고 이 층을 얹는** 순서입니다.
    - **돌리는 법:** PR에 `@coderabbitai review` **한 줄만** 담은 댓글, 또는 봇 코멘트의 **`🔍 Trigger review` 체크박스**. ⚠️ 같은 댓글에 설명을 붙이면 봇이 채팅으로 답하고 **리뷰를 접수하지 않습니다**(슬롯은 소모). 설명은 별도 댓글로. 잔량은 `@coderabbitai rate limit`.
    - **🚧 머지 기준은 리뷰 본문입니다.** `Review triggered`는 접수 확인일 뿐입니다. `Actionable comments posted: N`이 있어도 **그 뒤 푸시한 커밋은 리뷰되지 않았습니다**(증분 리뷰) — 재트리거하세요. ⚠️ **그 판정 문구가 아예 없는 리뷰도 있습니다** — actionable 0 + nitpick 1 조합은 문구 없이 니트픽 본문만 실립니다(#166 실측). 문구 유무로 "리뷰 안 됨"을 단정하지 말고 `keepcon-review-gate`의 3b(마커 + 리뷰 객체 두 소스)로 판정하세요.
    - **⚠️ CodeRabbit 체크의 green은 정보가 없습니다.** 리뷰 여부와 무관하게 항상 `pass`라 게이트로 쓸 수 없습니다. 판정은 [`keepcon-review-gate`](.claude/skills/keepcon-review-gate/SKILL.md)의 **6분기** 명령으로 합니다(`REVIEWED`/`RATE_LIMITED`/`NOT_TRIGGERED`/`CHAT_ONLY`/`ACK_ONLY`/`NONE`). ⚠️ `CHAT_ONLY`(봇이 답했으나 리뷰 명령으로 접수조차 안 됨)를 **리뷰 완료로 읽지 말고**, `ACK_ONLY`(접수됨 — 리뷰가 도는 중일 수 있음)는 **즉시 재트리거하지 마세요**. 각 상태의 처방은 스킬이 정본입니다 — 리뷰 본문이 `comments`가 아니라 `reviews`에 실리는 PR이 있어 눈으로 세면 오판합니다.
...
실제 사용자 두 명을 Auth 에뮬레이터에 만들고 그 토큰으로 Firestore에 요청해, 남의 기프티콘 조회·`ownerId` 위조·비멤버의 그룹 조회·**비멤버의 무단 합류**·**멤버의 권한 위조**(자기 role을 방장으로, 방장 강퇴, 그룹 메타데이터 변경)가 **실제로 차단되는지** 확인합니다.

이 검증은 CI의 **`Firestore rules`** 잡에서도 자동으로 돌아갑니다. Dart 테스트는 in-memory 구현을 쓰므로 규칙 계층의 결함을 원리상 잡지 못합니다 — 실제로 비멤버가 초대 없이 그룹에 합류하고 스스로 방장이 될 수 있던 구멍이 이 방식으로만 드러났습니다.
...
| `--android-package-name` | CLI가 프로젝트 파일에서 패키지명을 추측합니다. 명시하면 최소한 **내가 어떤 이름으로 등록하는지 커밋에 남습니다** — 위 사고처럼 브랜치 상태에 따라 값이 조용히 달라지는 걸 리뷰에서 잡을 수 있습니다. |
...
> ℹ️ **네이티브 설정 파일(`google-services.json`)을 쓰지 않습니다.** 그 파일은 네이티브가 `[DEFAULT]` FirebaseApp을 먼저 만들어 **백엔드를 하나로 고정**하는데, KeepCon은 emulator·dev·prod를 실행 시 고르기 때문입니다(파일이 있으면 나머지 둘이 `[core/duplicate-app]`으로 죽습니다). Android도 web과 똑같이 **생성된 Dart 옵션만** 씁니다 — 자세한 근거는 [`android/app/build.gradle.kts`](android/app/build.gradle.kts) 주석에 있습니다.
>
...
> ⚠️ **알려진 한계 — 교차 백엔드 링크는 열리되 실패합니다.** 매니페스트는 dev·prod 호스트를 빌드와 무관하게 **둘 다** 받습니다(Android 11 이하가 필터 안 모든 host의 검증을 요구하기 때문입니다). 그래서 dev 링크를 prod APK가 열면 앱은 뜨지만 토큰이 그 백엔드에 없어 **잘못된 자격증명 안내**가 뜹니다 — 링크는 멀쩡한데 원인을 정반대로 말합니다. 토큰이 128비트 랜덤이라 **오합류는 일어나지 않습니다**(fail-closed). 근본 해결은 빌드 플레이버별 `manifestPlaceholders`로 host를 하나만 넣는 것입니다.
>
