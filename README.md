...
`tool/emulators.sh`로 띄우면 [`emulator-seed/`](emulator-seed)에 커밋된 계정이 **이미 들어 있는 상태**로 시작합니다. 각자 회원가입할 필요가 없고, 팀원 전원이 **같은 uid**를 쓰므로 "방장 A가 만든 그룹에 파티원 B가 참여" 같은 시나리오를 그대로 공유할 수 있습니다.
