...
- **코드 리뷰(3층 중첩):** ①CI(`Format·Analyze·Test`) 게이트 ②PR마다 **CodeRabbit** 리뷰(**수동 트리거 필요** — 아래 참조) ③Claude Code로 작업 시 **코드 변경을 커밋하기 전에 `/code-review`로 자체 리뷰·수정 후 커밋**. 릴리스 전 `/security-review`.
  - **⚠️ CodeRabbit은 이 저장소에서 자동으로 돌지 않는다 — 반드시 트리거하라.** CodeRabbit은 **스타 10개 미만 저장소의 리뷰를 수동 트리거로 전환**했다(`Reviews should be triggered manually for repositories with fewer than 10 stars`). KeepCon은 여기 해당하므로, PR을 열어도 봇은 "Review available on request" 안내만 남기고 **리뷰하지 않는다**.
    - **트리거:** PR에 `@coderabbitai review` 댓글을 달거나, 봇 코멘트의 **`🔍 Trigger review` 체크박스**를 클릭한다. 봇이 `Review triggered`로 답하면 걸린 것이고, 리뷰 본문은 몇 분 뒤 올라온다.
    - **왜 규칙으로 박아 두는가:** 이 상태를 모르면 **봇이 조용한 것을 "지적 없음"으로 읽는다** — 3층 방어 중 2층이 비어 있는데 아무도 모르는 채 머지된다. 실패(red)보다 나쁜 것이 "돌지 않았는데 통과로 보이는 것"이다. PR #90에서 실제로 그럴 뻔했다.
    - **근본 해결:** 스타가 10개를 넘으면 자동 리뷰로 돌아온다. 그때 이 항목을 지워라.
  - **검증 실패 시 수정 루프(필수):** PR을 올린 뒤 CI(`Format·Analyze·Test` + `SSOT guard`)와 CodeRabbit 결과를 확인한다. **CodeRabbit은 먼저 트리거한 뒤 결과를 기다린다**(위 참조 — 안 돌린 채 "지적 없음"으로 넘기지 않는다). **하나라도 실패(red)하면 머지하지 말고**, 원인을 진단해 수정 → 커밋 → 푸시를 **전부 green이 될 때까지 반복**한다(빨간 PR을 방치하거나 머지하지 않는다). CodeRabbit이 남긴 유효 지적도 같은 루프로 반영한다. Claude Code로 작업 시 이 수정 루프를 **자동 수행**하며, 실패를 사용자에게 떠넘기지 않는다.
    - **강제 근거:** CI·CodeRabbit은 브랜치 보호 룰셋의 **필수 상태 체크(required status checks)** 로 등록되어 있어 **red 상태로는 머지 자체가 불가**하다 — 즉 "빨간 채로 방치"는 규칙 위반 이전에 **애초에 '작업 완료'가 성립하지 않는다**(게이트가 결과를 강제, 규칙이 행동을 안내).
      - **⚠️ CodeRabbit 쪽 강제는 지금 확인이 필요하다.** PR #90은 CodeRabbit이 **한 번도 돌지 않은 시점에** GitHub API가 `mergeable_state: clean`을 돌려줬다. 필수 체크가 미완료면 `blocked`여야 하므로, `CodeRabbit / Review`가 실제로는 필수 체크로 안 걸려 있을 가능성이 크다. 그렇다면 **수동 트리거를 깜빡한 PR이 게이트에 막히지 않고 그대로 머지된다** — 위의 "봇이 조용한 것을 통과로 읽는" 사고가 규칙만으로는 안 막힌다는 뜻이다. 룰셋에서 필수 체크 목록을 확인하고, 없으면 추가하거나 이 문장을 사실에 맞게 고쳐라.
