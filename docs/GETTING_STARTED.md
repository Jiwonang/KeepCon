...
3. 다른 한 명이 그 코드로 **참여 요청** — "참여 요청을 보냈어요"가 뜨면 접수된 것입니다. **아직 멤버가 아닙니다**
4. **방장이 승인해야 멤버가 됩니다.** 방장은 공유 탭 → 그 그룹 카드 → **그룹 상세** → **승인요청목록** 버튼을 눌러야 대기 목록이 보입니다(버튼은 대기 0건에도 남아 있고, 요청이 있으면 건수 뱃지가 붙습니다. 홈·공유 탭에는 도착 알림이 뜨지 않습니다). 승인하면 요청자의 공유 탭에 그룹이 나타납니다
5. 한 명이 기프티콘을 그룹에 공유 → **상대 화면에 뜨는지 확인**
6. 상대가 사용 처리 → **원래 주인에게 알림·사용이력이 반영되는지 확인**
...
| 에뮬레이터가 뜨다 마는데 Java 얘기가 나옴 | **버전이 아니라 메시지로 가르세요.** ⓐ ``no longer supports Java version before 21`` → `java -version`을 확인하고 21 미만이면 1번 항목대로 올리세요. **21 이상인데도 이 메시지가 나면** `firebase-tools`가 버전 문자열을 못 읽은 것이니(파싱 실패는 `-1`로 취급돼 같은 메시지가 납니다) `java -version` 출력 전문을 공유해 주세요. ⓑ ``Could not spawn `java -version` `` → 그 셸에서 java가 안 보이는 것. `java -version` 자체가 안 되면 미설치(1번 항목), 되는데도 이 메시지가 나면 셸 문제 — 바로 위 항목을 보세요. ⓒ ``has exited with code`` / ``was killed by signal`` → 위 어느 쪽도 아닙니다. Java 설치가 깨진 것이니 재설치하세요. |
