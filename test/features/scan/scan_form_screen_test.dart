...
/// (⚠️ `GifticonFormScreen`은 이제 **공개**라 직접 pump할 수 있지만, 그러지 않는다 —
/// 이 폼은 `GifticonFormController`의 세션(`startWith`)을 전제하므로 직접 pump하면
/// 실제 경로가 만들지 않는 상태에서 검증하게 된다. 예전에는 private이 그것을 막아
/// 줬고, 지금은 이 규약이 막는다. 경로를 통째로 지나야 이 파일이 막으려는
/// 회귀를 잡을 수 있다.)
///
/// ## 검증 대상
