...
void main() {
  ...
  test('저장물에는 로컬 이미지 경로가 실리지 않는다 — 미리보기용 폼 상태에는 남는다', () async {
    ...
    // 표시되지 않고(홈 타일은 이미지를 아예 그리지 않고 브랜드 색 블록을 쓴다 —
    // card_clock_test가 단언. 상세 화면도 이미지를 싣지 않는다 —
    // gifticon_detail_page.dart doc) OS가 임시 디렉터리를 청소하면 무효가
    // 되는데, Firestore 구현은 그것을 문서에 그대로 쓴다 — 즉 채우면 깨진
    // 경로가 쌓인다.
    //
