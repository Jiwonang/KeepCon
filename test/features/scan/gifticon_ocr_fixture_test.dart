...
///
/// ## ⚠️ 출처가 둘로 나뉜다 — 오인식 정도가 다르다
///   - `_kakaoGiftOcr`·`_giftishowOcr` — **실제 소지 기프티콘의 스크린샷**
///   - `_kakaoBarcodeCardOcr`·`_dominoBarcodeCardOcr` — **웹에서 찾은 이미지**
///
/// 웹 이미지는 압축·리사이즈를 거쳐 **실제 폰 스크린샷보다 OCR이 더 많이 틀린다.**
/// 도미노 원문의 `Demins's`·`Pirza`·`도미노파자`가 그 흔적이고, 상품명이 **어디서**
/// 끊기는지도 이미지 폭에 달려 있어 실제 화면과 다를 수 있다.
///
/// 그렇다고 못 쓸 픽스처는 아니다 — 스캔 페이지의 두 경로 중 **카메라 촬영**은
/// 화질이 나쁜 것이 정상이라 오히려 그쪽을 대표한다. 다만 "갤러리에서 고른 깨끗한
/// 스크린샷의 전형"으로 읽으면 안 된다. 특히 **잘림의 빈도**를 이 둘로 추정하지
/// 말 것 — 끊기는 지점은 렌더 폭의 함수이지 파서의 함수가 아니다.
///
/// ## 지어낸 입력으로는 안 되는 이유
...
///
/// ## 상품명 잘림 — 고쳤다 (두 픽스처가 규칙을 정했다)
/// 예전에는 상품명이 여러 줄로 쪼개지면 파서가 **첫 줄만** 써서, 사용자에게 문장
/// 중간에서 끊긴 값이 저장됐다. 픽스처 둘이 **서로 다른 방식으로** 끊겨 있었고,
/// 그 차이가 규칙을 결정했다.
///
///   - 노랑통닭: `…콜라1.` / `25L` — **마침표**(`1.25L`의 소수점)에서 끊김
///   - 도미노피자: `슈퍼슈프림(오리지` / `널)L+…` — **단어 한가운데**, 구두점 없음
///
/// 한 장만 봤다면 "마침표로 끝나면 이어붙인다"를 세웠을 것이다 — 노랑통닭에서는
/// 그게 정답을 낸다. 그런데 마침표가 소수점이라서지 문장이 이어져서가 아니고,
/// **도미노피자에는 그 규칙이 걸릴 자리가 아예 없다.** 한 장은 틀린 규칙을 옳아
/// 보이게 만들고, 둘째 장이 그것을 반증한다.
///
/// 그래서 구두점이 아니라 **경계**로 푼다 — 연속된 후보 줄을 구분자 없이
/// 이어붙이고, 금액·날짜·바코드·유효기간·**필드 라벨**에서 멈춘다. 라벨 판별이
/// 새로 필요했다: 노랑통닭은 상품명 다음 줄이 `교환처`라 거기서 안 멈추면
/// `…콜라1.25L교환처`가 되고, 기프티쇼는 `교환처 : 스타벅스`가 딸려 붙는다.
///
/// 반례 자체는 픽스처 무결성 단언으로 지킨다 — 누가 상수를 정리하며 두 줄을
/// 합치면 규칙의 근거가 조용히 사라지기 때문이다.
///
...
///
/// 출처: **웹에서 찾은 이미지**(구글). 상단 ⚠️ 참조 — 이 픽스처의 오인식이
/// 유난히 심한 것도 그 때문으로 보인다.
///
...
void main() {
  ...
  group('카카오톡 바코드 카드 원문 — 사전 밖 브랜드', () {
    ...
    test('두 줄로 쪼개진 상품명을 이어붙이고 라벨에서 멈춘다', () {
      final GifticonOcrResult result = parser.parse(_kakaoBarcodeCardOcr);

      // `(오리지널/순살) 3종세트+콜라1.` + `25L`. 그 다음 줄이 `교환처`라
      // 거기서 멈추지 않으면 `…콜라1.25L교환처`가 된다.
      expect(result.productName, '(오리지널/순살) 3종세트+콜라1.25L');

      // ⚠️ `유효기간`은 `_isExpiryRelatedText`가, `교환처`·`주문번호`는
      // `_isFieldLabelLine`이 막는다. 단, 이 픽스처에서는 브랜드가 0번·상품명이
      // 1번 줄이라 **상품명 경계로서만** 걸리고 후보 순서로는 닿지 않는다 —
      // 라벨이 브랜드보다 앞서는 배치는 아직 수집하지 못했다.
      expect(result.productName, isNot(contains('교환처')));
      expect(result.productName, isNot(contains('유효기간')));
      expect(result.productName, isNot(contains('주문번호')));
  ...
  group('도미노피자 바코드 카드 원문 — 잘림이 단어 한가운데', () {
    test('단어 한가운데서 쪼개진 상품명을 이어붙이고 바코드에서 멈춘다', () {
      ...
      // `슈퍼슈프림(오리지` + `널)L+갈릭&윙스+콜라`. 다음 줄이 바코드라 거기서
      // 멈춘다 — 그 뒤의 `L`·`Demins's`·`Pirza`는 딸려 오지 않는다.
      expect(result.productName, '슈퍼슈프림(오리지널)L+갈릭&윙스+콜라');
      expect(result.productName, isNot(contains('Demins')));
      expect(result.productName, isNot(contains('Pirza')));
    });

    test('잘린 자리에 붙잡을 구두점이 없다 — 구두점 기반 규칙을 반증한다', () {
      // 앞 픽스처는 `…콜라1.` / `25L`로 마침표에서 끊겨, "마침표로 끝나면
      // 이어붙인다"가 정답을 내는 것처럼 보였다. 여기서는 `오리지` / `널)`로
      // 단어 한가운데가 끊긴다 — 그 규칙이 **걸릴 자리가 없다.** 그래서 파서는
      // 구두점이 아니라 **경계**(금액·날짜·바코드·유효기간·필드라벨)로 멈춘다.
      //
      // 이 단언은 파서가 아니라 **픽스처 자체**를 고정한다. 누가 상수를 정리하며
      // 두 줄을 합치면 이 반례가 조용히 사라지고, 상단 「상품명 잘림」 문단이
      // 규칙의 근거를 잃는다.
