...
/// 먼저 울기 때문이다.
///
///   - 독립 회귀 가드 8건: 종합 4건(카카오 상세·기프티쇼·노랑통닭·도미노) ·
///     금액 인과 1건 · 상품명 시작 1건 · 무결성 2건
///   - 종합 테스트에 종속(문서 + 트립와이어) 9건: 도장 · 시계 · 유효기간 ·
///     사전 · 라벨분리 · 로고 · 주문번호 · 구두점반증 · 라벨한줄
///
/// 뮤테이션으로 확인한 분류다. 예컨대 `pepsi`·주문번호 테스트는 어떤
/// 뮤테이션에서도 죽지 않는다 — 노랑통닭 픽스처는 브랜드가 첫 줄이라 로고나
/// 숫자열이 이길 수 없기 때문이다. 그 둘이 이기는 배치의 원문을 아직 수집하지
/// 못했다. `구두점반증`은 파서가 아니라 **픽스처 상수 자체**를 고정하므로
/// 설계상 파서 뮤테이션으로 죽지 않는다.
///
