...
class _FirebaseBackend implements ShareBackend {
  ...
  Future<void> passTime(Duration d) async {
    ...
    //
    // 세 곳을 모두 당긴다: 그룹 문서의 두 정본과 조회 문서 둘. 한 곳만 당기면 그룹과
    // 조회 문서의 만료가 어긋나 **현실에 없는 상태**가 되고, 그 위에서 통과한 테스트는
    // 아무것도 보증하지 않는다.
    //
    // ⚠️ 다만 **지금 케이스가 읽는 것은 조회 문서뿐이다**(`requestToJoin`은 그룹 문서의
    //    만료 필드를 아예 안 본다). 그래서 그룹 문서 쪽을 빼도 현재 18건은 통과한다 —
    //    이 줄은 뒤에 붙을 케이스를 위한 선제 조치이지 지금 무언가를 지키고 있지 않다.
