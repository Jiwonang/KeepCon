...
void main() {
  ...
  group('requestToJoin — 자격증명 종류(isCode) 판정', () {
    ...
    //    먼저 터져, 양쪽에서 물고 있으므로 거짓 통과로는 빠지지 않는다.
    ...
    test('1차 조회는 코드 컬렉션을 먼저 본다', () {
      // 이 한 줄이 정하는 것은 **1차 조회 대상**이다(`isCode`의 초기값은 바로 아래
      // 케이스가 따로 문다 — 같은 `codeShaped`를 읽지만 다른 줄이다). 뒤집으면 살아
      // 있는 6자리 코드가 1차·폴백 모두 `_inviteTokens`를 보게 되어 통째로 '없는
      // 코드'가 되고, "코드를 먼저 본다"는 전제가 무너져 5분 창이 24시간 창에
      // 흡수된다(계약 v3.12). 폴백 단언들의 창은 폴백 앵커에서 시작하므로 이 줄을
      // 지나친다 — 여기서 막는다.
      expect(
    ...
    test('종류 판정을 만드는 줄과 읽는 줄을 함께 못박는다', () {
      // 1차 조회 줄과 폴백 앵커 **사이 두 줄**은 다른 단언이 하나도 물지 않는다 —
      // 거기 심은 변형 셋이 667건 전부를 통과했다(리뷰에서 실측). 무엇이 새는지:
      //
      //  - 초기값을 `false`로 굳히면 **폴백을 안 타는 경로**(살아 있는 6자리 코드)의
      //    만료가 "만료된 초대 링크"로 안내된다 — 방장이 링크를 재발급해도 그 사람이
      //    쥔 코드는 그대로라 다음 행동이 틀린다.
      //  - `true`로 굳히면 만료된 22자 링크가 "만료된 초대코드"로 안내된다 —
      //    **이 PR이 고친 오분류 그 자체**다.
      //  - 진입 조건의 `!`가 빠지면 찾아낸 코드 문서를 없는 토큰 문서로 덮어써
      //    모든 초대코드가 '없는 코드'가 된다.
      final String body = bodies['requestToJoin']!;
      expect(body, matches(RegExp(r'bool\s+isCode\s*=\s*codeShaped\s*;')),
          reason: '초기값을 상수로 굳히면 폴백을 안 타는 경로의 종류 판정이 통째로 틀린다');
      expect(
          body,
          matches(
              RegExp(r'if\s*\(\s*!\s*credDoc\.exists\s*&&\s*codeShaped\s*\)')),
          reason: '폴백 진입 조건이 뒤집히면 살아 있는 코드 문서를 없는 토큰 문서로 덮어쓴다');
      // ⚠️ 위 셋은 `codeShaped`를 **읽는** 자리만 본다 — 그것이 무엇인지 정하는 줄은
      //    따로 있고, 뒤집으면 1차 조회·초기값·폴백 진입이 **한꺼번에** 틀리면서 위
      //    단언들은 전부 통과한다(실측: 668건 전부 green). 판정을 만드는 자리다.
      //    in-memory의 같은 줄은 행위 테스트 6건이 물고 있어, 이 그룹이 메우려던 두
      //    구현 간 비대칭이 정작 **입력 쪽에** 남아 있었다.
      expect(
          body,
          matches(RegExp(
      ...
      // 대입은 선언과 폴백 강등 둘뿐이어야 한다.
      expect(RegExp(r'isCode\s*=(?!=)').allMatches(body).length, 2,
          reason: '`isCode` 대입이 둘이 아니다 — 초기값이나 폴백 판정이 덮어써진다');
    });
