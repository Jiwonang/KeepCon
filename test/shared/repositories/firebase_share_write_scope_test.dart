...
///
/// 그런데 이 배정은 **지금 있는 테스트로는 보이지 않는다.** 매퍼 본체는 순수 static
/// 함수라 직접 검증하지만, 호출부의 등급을 되돌려도 매퍼 테스트는 전부 통과한다.
///
/// ⚠️ **"Firestore 없이 실행할 수 없어서"가 아니다.** `fake_cloud_firestore`가 들어와
///    호출부를 실제로 돌릴 수 있게 됐다(`share_repository_contract_test.dart`가 그 위에서
///    `createGroup`·`issueInviteCode`·`getGroupById`·`requestToJoin`을 구동한다).
///    남은 제약은 "손상 문서를 심어 각 등급의 거부를 **행위로** 고정하는 케이스를 아직 안
///    썼다"뿐이다. 아래 검사는 그때까지의 임시방편이지 소스 검사를 **늘릴** 근거가 아니다
///    — 소스 검사는 무엇을 '같다'고 볼지 매번 명시해야 해서 수렴하지 않는다(PR #154에서
///    다섯 라운드 연속 새 거짓 통과가 나왔다).
///
/// 그 구멍을 당분간 소스 검사로 메운다 — 트랜잭션이 `_groupToDoc`으로 문서 전체를 되쓰면 반드시
