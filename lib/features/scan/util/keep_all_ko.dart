...
///
/// ⚠️ 두 가지 제약:
/// - 어절 하나가 줄 폭보다 길어도 **넘치지는 않는다** — 엔진(SkParagraph)이 줄보다
///   긴 어절을 줄 폭에 맞춰 강제로 끊는다(줄 폭이 글자 하나보다는 넓을 때).
///   남는 제약은 끊기는 **자리**다: `/`·`-` 같은 자연스러운 지점이 **아니게 된다** —
///   원문이라면 그 뒤에서 끊겼을 자리까지 이 함수가 막으므로 긴 URL·식별자는
///   `https:/` + `/console…`처럼 임의 위치에서 갈린다. 얻는 것이 없을 뿐 아니라
///   있던 끊김 자리를 잃는다 — 그 손실은 URL만 담은 문구에 국한되지 않는다.
///   한글 문장에 URL이 끼어도 마찬가지다: scan 스낵바가 예전에 쓰던 `'…: $e'`
///   꼴 스낵바는 272px에서 자연스러운 끊김 자리 5개가 **1개**로 줄었다. 그러니
///   원시 예외·URL을 품는 문구는 어절 보호를 손보기보다 **문구에서 그것을
///   걷어내는 쪽**이 낫다(`pages/gifticon_form_page.dart`의 `ScanSubmitFailure`
///   분기 주석이 그 전환을 기록해 두었다).
///   재어서 못박아 두었으니(`keep_all_ko_test.dart`) 이 문장을 고칠 때는 그
///   테스트를 먼저 보라.
