...
class GifticonFormController extends StateNotifier<GifticonFormState> {
  ...
  Future<Gifticon?> submit() async {
    ...
    if (activeCount >= UserPlan.freeGifticonLimit) {
      ...
      if (plan == null) {
        // **처방이 원인을 가리켜야 한다.** 여기 오는 가장 흔한 경우는 오프라인이다 —
        // 계약이 이 조회를 서버 확인으로 못박아(`Source.server`) 캐시로 폴백하지
        // 않으므로, 연결이 없으면 이 경로는 **반드시** 실패한다. 그런데 "잠시 후"는
        // 기다리면 낫는다는 뜻이라 오프라인 사용자에게는 틀린 처방이다(연결을
        // 되살리기 전까지 몇 번을 다시 눌러도 같은 자리에서 막힌다).
        //
        // 그렇다고 원인을 오프라인으로 단정하지도 않는다 — 구현은 권한 오류·서버
        // 오류도 같은 예외 하나로 던지므로 여기서는 구분할 수 없다. 연결 확인을
        // 앞세우되 "다시 시도"를 남겨 두 경우를 모두 덮는다.
        //
