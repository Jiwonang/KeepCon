...
/// **먼저** 어느 쪽을 조회할지 고르는 힌트다. 지금 발급되는 링크 토큰은 base64url 22자라
/// 이 검사를 통과하지 못한다.
///
/// ⚠️ **통과했다고 코드인 것은 아니다.** v3.0(128비트) 이전의 링크 토큰은 6자리 숫자였고
/// 그 값들이 아직 살아 있다(팀 공용 시드 `482913`, 데모 시드, 레거시 Firestore 문서).
/// 즉 이 함수가 말할 수 있는 것은 "코드일 **수도** 있다"까지다
/// (`test/shared/util/invite_code_test.dart`의 레거시 케이스가 그것을 못박는다).
///
/// 그래서 **"이것이 코드였는가"의 정본은 이 함수가 아니라 저장소의 폴백 조회 결과다** —
