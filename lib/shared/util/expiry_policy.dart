...
/// 만료 임박 알림을 띄우는 시점(만료 며칠 전). 내림차순으로 둔다.
///
/// 첫 값이 [expirySoonDays]와 같아야 화면의 "만료 임박"과 첫 알림이 같은 순간에 맞는다
/// — 홈이 빨갛게 변하는 날 알림도 오는 것이 사용자에게 자연스럽다. 이후 D-3·D-1은
/// "봤지만 잊은" 경우를 위한 것이다(한 번만 보내면 그 하루를 놓치면 그대로 만료된다).
const List<int> expiryNotifyLeadDays = <int>[expirySoonDays, 3, 1];

/// 만료 임박 알림을 띄우는 시각(로컬 기준 시, 0~23).
///
/// 오전 9시 — 출근/등교 직후라 "오늘 이걸 써야겠다"는 판단으로 이어지기 쉽다. 새벽에
/// 울려 소리로 깨우는 일도 없다.
const int expiryNotifyHour = 9;
