/// KeepCon 공유 유틸 — 한글 조사 자동 선택.
///
/// 알림·스낵바 등 동적으로 조립하는 문구에서 `을(를)` 같은 fallback 표기 대신
/// 앞 단어의 받침 유무로 조사를 골라 자연스러운 한국어를 만든다.
///
/// 프로토타입(`lib/features/share/data/korean_particle.dart`)에서 승격한 **범용 유틸**이다.
/// share 도메인에 국한되지 않는 순수 언어 유틸이므로 `lib/shared/util/`에 두어 어느 페이지든
/// 소비할 수 있게 한다(모델/Repository 계약과 달리 도메인 결합이 없다).
///
