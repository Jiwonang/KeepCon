...
///
/// ## 소비 — 이 provider로 **`.report(...)`를 직접 부르지 마라**
///
/// 잡는 자리에서는 항상 `report_handled_failure.dart`의 두 함수 중 하나를 거친다(고르는
/// 기준은 그 파일 머리말 — "보고 전 `await` 동안 폐기될 수 있는가"). [WidgetRef.read]는
/// 위젯이 폐기된 뒤 호출되면 **던지는데**(assert가 아니라 무조건 `StateError` — release
/// 포함), 이 진단이 필요한 자리는 정확히 `await` 뒤의 catch라 그 사이 위젯이 사라지는
/// 일이 흔하다. 그때 보고가 던지면 **catch 블록의 나머지(사용자 안내)가 실행되지
/// 않는다** — share 화면이 `catch (_)`까지 넓혀 가며 없앤 "아무 안내도 없이 버튼이 죽은
/// 것처럼 보인다"가 되살아난다.
///
/// ```dart
/// // 형태 예시다. 이 자리는 위 기준으로는 "폐기될 수 있다"(그룹 상세 본문)지만
...
/// ```
///
/// 이 provider를 직접 읽는 곳은 ①`reportHandledFailure` 래퍼 ②`reportHandledFailureTo`에
/// 넘기려고 `await` **이전에** 리포터를 잡아 두는 자리, 둘뿐이어야 한다.
///
/// ## 원격 수집으로 갈아끼우기
///
