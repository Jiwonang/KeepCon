...
abstract class ShareRepository {
  ...
  /// 그 두 갈래에서는 개인 목록에 `available` + 새 만료일인 원본이 남아, 이미 소진된
  /// 기프티콘이 더 오래 쓸 수 있는 것처럼 보인다. 지금은 그 상태를 되돌리지 않는다.
  ///
  /// ⚠️ 그 두 갈래에는 회귀 테스트가 없다 — 계약 스위트의 in-memory `GifticonRepository`
  /// 에는 소유자 가드가 없고 `firebase(fake)`는 보안 규칙을 평가하지 않아, 두 백엔드
  /// 모두 교차-멤버 권한 실패를 표현할 수 없다.)
  ///
