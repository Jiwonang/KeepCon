...
abstract class ShareRepository {
  ...
  //
  // 옛 가드들이 간 자리:
  // - 없는 토큰·만료 → [requestToJoin]
  // - 중복 요청 → [requestToJoin]인데 **조용한 반환이 아니다.** 대기 중인 요청이 있으면
  //   [JoinRequestAlreadyPendingException]으로 그 요청을 실어 알린다(저장소는 여전히
  //   아무것도 쓰지 않는다 — 바뀐 것은 호출부가 그 사실을 안다는 것뿐이다).
  // - 정원 → [approveJoinRequest] (대기자가 자리를 선점하지 못하게 승인 시점으로 옮겼다)
  // - **이미 멤버 → [requestToJoin]인데 의미가 바뀌었다.** 옛 `joinGroup`은 no-op으로
  //   현재 그룹을 반환했고 그 판정이 만료·정원보다 **우선**이었다. 지금은
  //   [AlreadyGroupMemberException]으로 거부하고 **만료 검사가 먼저** 온다 — 이미
  //   멤버라도 링크가 24시간을 넘겼으면 '만료'로 거부된다(멤버가 단톡방에 남은 자기
  //   링크를 누르는 흔한 경로다).
  //   거부 자체는 v2.9에서 들어왔고 타입을 좁힌 것은 그 뒤다. 여기서는 대응 관계만 기록한다.
  ...
  /// ⚠️ **예외가 하나 있다** — 그 창에서 항목이 [ShareStatus.used]가 되면 위 가드가
  /// 재연장을 영구히 거부하므로 복구가 통하지 않는다. 원본만 새 만료일을 가진 채 남고,
  /// **그 원본이 `used`로 따라가리라는 보장은 없다** — 바로 위에 적은 대로 `markUsed`의
  /// 원본 동기화는 best-effort라 ①행위자가 원본 소유자가 아니면(교차-멤버 사용) 권한으로,
  /// ②원본이 아직 `expired`면 전이 표에 `expired → used`가 없어서 조용히 건너뛴다.
  /// ②는 드문 갈래가 아니다 — **만료된 기프티콘이 이 기능의 주 사용 사례**다.
  /// 그대로 두면 개인 목록에 `available` + 새 만료일인 원본이 남아, 이미 소진된
  /// 기프티콘이 더 오래 쓸 수 있는 것처럼 보인다. 그래서 **거부하면서 원본을
  /// `GifticonStatus.used`로 맞춘다**(구현의 보정 경로). 만료일까지 되돌리지는 못하지만
  /// (계약에 앞당기는 API가 없다) 사용 완료된 기프티콘의 만료일은 표시에 쓰이지 않는다.
  ///
  /// ⚠️ 그 두 갈래를 **만드는** 조건에는 회귀 테스트가 없다 — 계약 스위트의 in-memory
  /// `GifticonRepository`에는 소유자 가드가 없고 `firebase(fake)`는 보안 규칙을 평가하지
  /// 않아, 두 백엔드 모두 교차-멤버 권한 실패를 표현할 수 없다. 보정의 **결과**(거부 뒤
  /// 원본이 `used`인 것)는 계약 스위트가 고정한다.)
  ///
