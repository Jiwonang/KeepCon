...
/// 불러주는 동안 창이 닫히는 일이 흔하고, 그때 "링크가 잘못됐거나 만료됐을 수 있어요"
/// 같은 뭉뚱그린 안내를 받으면 사용자는 코드를 잘못 들었는지 시간이 지난 것인지 알 수
...
class InviteExpiredException extends StateError {
  ...
  InviteExpiredException(this.credential)
...
abstract class ShareRepository {
  ...
  /// [issueInviteCode]가 발급한 **6자리 초대코드**. 둘은 모양으로 구분되며
  /// (`isWellFormedInviteCode`), 구현체가 해당하는 조회 문서를 찾아 그룹으로 해석한다.
  /// 화면 입력란이 하나이므로 계약도 하나로 받는다 — 사용자에게 "지금 넣는 것이 링크인지
  /// 코드인지" 고르게 하는 것은 전달 수단의 차이를 사용자 몫으로 떠넘기는 것이다.
  ///
