...
///
/// ## 둘 중 무엇을 쓰는가 — 기준은 "이 `await` 동안 폐기될 수 있는가"
///
/// 위젯 종류(시트냐 페이지냐)가 기준이 **아니다.** 묻는 것은 하나뿐이다 —
/// **보고 전에 기다리는 `await` 동안 이 위젯이 사라질 수 있는가.**
///
/// - **있다** → 리포터를 `await` **이전에** 잡아 [reportHandledFailureTo]로 보고한다.
///   [reportHandledFailure]는 그때 `ref.read`가 던져 **아무것도 남기지 않는다**
///   (사용자 안내는 지켜지지만 진단이 조용히 빈다).
/// - **없다** → [reportHandledFailure]로 충분하다.
///
/// "있다"의 흔한 모양:
///
/// - 사용자가 닫을 수 있는 시트·다이얼로그(`isDismissible` 기본값, 뒤로 가기).
/// - 스트림 상태에 따라 unmount되는 본문(그룹 상세 — loading/error 방출).
/// - 스트림 목록의 **행** — 다른 기기·다른 사용자의 변경으로 행이 목록에서 빠진다.
/// - `unawaited`로 띄운 작업 — 호출한 화면이 결과를 기다리지 않고 떠날 수 있다.
///
/// **가장 확실한 신호는 코드 자신이다: 같은 catch 안에서 `mounted`를 검사하고 있다면
/// 그 자리는 이미 "폐기될 수 있다"고 증언한 것이다.** 그 옆에서 [reportHandledFailure]를
/// 부르면 자기모순이다 — 검사가 필요한 바로 그 상황에서 보고가 빈다.
///
/// ⚠️ 기존 [reportHandledFailure] 호출부를 **이 기준으로 일괄 전환하지 않기로 했다.**
/// 틀린 쪽을 골라도 사용자에게 보이는 것은 없고(안내 자원은 `await` 전에 잡아 둔다)
/// 개발자 로그에서 "화면을 떠난 뒤 실패"라는 좁은 표본만 빈다 — 기계적 대량 변경의
/// 값어치에 못 미친다. 새 코드는 이 기준으로 고르고, 기존 자리는 **다른 이유로 손댈 때**
/// 함께 맞춘다. 단, 위 자기모순(catch 안 `mounted` 검사)이 드러난 자리는 이 기준을 박을
/// 때 함께 고쳤다.
library;
...
/// 진단 쪽은 조용히 비는 것이다.
///
/// 그런데 그 창이 **정상 경로**인 자리가 있다 — `await` 동안 위젯이 폐기될 수 있는
/// 자리(판단 기준과 흔한 모양은 파일 머리말의 "둘 중 무엇을 쓰는가"). 하필 거기가 원격
/// 진단이 가장 필요한 표본이다 — 사용자가 화면을 떠난 뒤의 백엔드 실패이고, 그것만
/// 로그에서 빠지면 표본이 조용히 편향된다("화면을 안 떠난 실패"만 모인다). 그런 자리는
/// 리포터를 `await` 이전에 잡아 이쪽으로 보고한다.
///
/// ```dart
