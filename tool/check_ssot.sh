...
# 뒤의 `\b` 때문에 매칭되지 않는다.
defs=$(grep -rEn "^[[:space:]]*([a-z]+[[:space:]]+)*(class|enum|mixin)[[:space:]]+_?(${SHARED_TYPES})\b" "$FEATURES_DIR" 2>/dev/null || true)
if [ -n "$defs" ]; then
...
prov=$(grep -rEn "^[[:space:]]*(late[[:space:]]+)?final[[:space:]]+([A-Za-z0-9_<>,.? ]+[[:space:]]+)?_?(${SSOT_PROVIDERS})[[:space:]]*=" "$FEATURES_DIR" 2>/dev/null || true)
if [ -n "$prov" ]; then
...
# 둘 다 통과):
#  3a) 반환 타입이 앞서는 선언 — 선택적 `static`, `void|dynamic|Future<...>|대문자 시작
#      타입`, 선택적 제네릭 파라미터(`foldSessionUser<T>(`)까지 포괄. `return`/`await`
#      같은 소문자 키워드는 타입 후보에 없어 호출문이 매칭되지 않는다.
...
fns_typed=$(grep -rEn "^[[:space:]]*(static[[:space:]]+)?(void|dynamic|Future<[^>]*>|[A-Z][A-Za-z0-9_<>,.? ]*)[[:space:]]+_?(${SSOT_FUNCTIONS})(<[^>]*>)?[[:space:]]*\(" "$FEATURES_DIR" 2>/dev/null || true)
fns_untyped=$(grep -rEn "^[[:space:]]*_?(${SSOT_FUNCTIONS})(<[^>]*>)?[[:space:]]*\([^;]*\)[[:space:]]*(async[[:space:]]*)?(\{|=>)" "$FEATURES_DIR" 2>/dev/null || true)
