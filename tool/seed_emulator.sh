...
#
# 왜 필요한가:
#   각자 회원가입해서 쓰면 uid가 달라 "방장 A / 파티원 B" 같은 시나리오를 팀원끼리
#   공유할 수 없다. 이 스크립트로 만든 계정을 `emulator-seed/`에 내보내 **git으로
#   커밋**하면, clone한 팀원 누구나 같은 계정·같은 uid로 즉시 로그인할 수 있다.
#
#   (개인 작업 데이터는 `tool/emulators.sh`가 `.emulator-local/`에 따로 유지한다 —
#    이 스크립트가 건드리는 커밋된 시드와는 **역할이 다르다**. 헷갈리지 마라.)
#
# 언제 실행하나:
